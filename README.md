...
Different parts of this repository are released under different terms; read the relevant license file for each:

- **Source code** — MIT License (`LICENSE-CODE`). Covers `standalone_pipeline/`, `evaluation_paper/analysis/`, and `evaluation_paper/annotation_review_app/`.
- **Source data** — CC BY-NC 4.0 (`LICENSE-DATA`). Covers the audio recordings, transcripts, dataset files, and human annotation files (`inputs/` audio, `evaluation_paper/data/`, `evaluation_paper/annotations/`, `evaluation_paper/manifest/`). The audio carries additional speaker-protection restrictions described in `LICENSE-DATA`.
- **Generated tables and figures** — CC BY 4.0 (`LICENSE-FIGURES`). Covers the aggregate outputs in `evaluation_paper/outputs/`, which contain no audio or identifiable personal data.

The manuscript itself is not included in this repository; it is distributed on arXiv under the arXiv.org perpetual, non-exclusive license.
